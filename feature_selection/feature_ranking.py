...
def main():
    ...
    if args.libsvm1_scale_file:
    ...
    elif args.plainfeatmat:
       
        df = pd.read_csv(args.plainfeatmat, sep = args.sep).fillna(-1)
        ...
        raise Exception("You must either use the --libsvm1_scale_file or --plainfeatmat flag!")
    ...
    if args.rownorm == True:
        ...
        print(X_dense)
    for model in tqdm([AdaBoostClassifier(), AdaBoostRegressor(), GradientBoostingClassifier(), GradientBoostingRegressor(), RandomForestClassifier(), RandomTreesEmbedding(), RandomForestRegressor(), ExtraTreesClassifier(), ExtraTreesRegressor()]):
